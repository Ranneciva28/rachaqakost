    ...
    @foreach($section->fields->sortBy('position') as $field)
        ...
        <label class="public-field {{ $wide?'wide':'' }}">
            ...
            @elseif($field->type==='file')
                @if($editable)<input type="file" name="files[{{ $key }}]" accept="image/jpeg,image/png,image/webp,application/pdf" @required($field->required&&!$upload)><small>JPG, PNG, WEBP, atau PDF · maksimal 25 MB. Foto akan diperkecil otomatis tanpa memotong isi.</small>@endif
